...
class Question3Visualizer:
    ...
    def plot_region_forecast(self, region_summary_df: pd.DataFrame) -> None:
        ...
        fig.suptitle("2024—2030年四大区域绿色交通发展趋势预测", fontsize=16, fontweight="bold", y=0.995, color=self.palette["ink"])
        self._add_caption(fig, "图3-2 四大区域在三种情景设定下的绿色交通发展趋势")
        fig.subplots_adjust(top=0.84, bottom=0.12, hspace=0.26, wspace=0.14)
